...
class RCM_Server(threading.Thread):
    ...
    def server_init(self):
        ...
        if (rf.FLAG_THREAD_STOP == True and rf.FLAG_RECV_FROM_NSG_STOP == True):
    ...
    def send_Waiting_Order_To_Robot(self):
        ...
        while (rf.FLAG_SERVER_CLOSED == False and rf.FLAG_THREAD_STOP == False):
            try:
                if (rf.FLAG_THREAD_SENDING_STATE == False):
                    if (rf.FLAG_SEND_ANGLE_DATA_TO_ROBOT == False):
                        if 'ROBOT' in rd.data_Tcp_Clinet_List:
                            self.STANDBY()
                    time.sleep(0.2)
                else:
                    if (rf.FLAG_START_SENDING_JOINT_ANGLE_DATA == True):
                        while (self.repeat_Count <= self.repetition):
                            if (rf.FLAG_HOME == True):
                                ...
                                self.repeat_Count = 0
                                break
                            elif (self.sended_Data_Number > (
                                    rd.motion_Row_Number - 1) and rf.FLAG_ROBOT_MOTION_DONE == True):
                                ...
                                self.sended_Data_Number = 0
                                self.repeat_Count += 1
                                # if ("ROBOT" not in rd.data_Tcp_Clinet_List):
                                rf.FLAG_SIMULATION = False
                            elif (self.sended_Data_Number > (
                                    rd.motion_Row_Number - 1) and rf.FLAG_ROBOT_MOTION_DONE == False):
                                pass
                            else:
    ...
    def server(self):
        ...
        print("└───────────────────────┘")
        if (rf.FLAG_SERVER_CLOSED == False and rf.FLAG_THREAD_STOP == False):
            try:
                rf.FLAG_SERVER_OPEN = True
                self.accept_Client()
            except Exception as e:
                print("Server has an error : ", e)
                rf.FLAG_SERVER_EMS_MSG = True
                self.send_To_All('D')
            ...
            try:
                if devices:
                    for device in devices:
                        if device == "ROBOT" and "ROBOT" not in rd.data_Tcp_Clinet_List:
                            rd.data_Tcp_Clinet_List[device] = self.client_Count
                            self.client_Count += 1
                            self.robot_Connected = 1
                        if device == 'FIXGUN' and 'FIXGUN' not in rd.data_Tcp_Clinet_List:
                            rd.data_Tcp_Clinet_List[device] = self.client_Count
                            self.client_Count += 1

                        # sql_List = self.rcm_sql.send_Product_Name() #sg_log 데이터 보냄
                        # if sql_List != rd.old_sql_List:
                        #     if len(sql_List) < 12:
                        #         sql_List_ = sql_List[:3] + '-' + sql_List[3:5] + "-" + sql_List[5:]
                        #         print("send product name : ", sql_List_)
                        #         RCM_Server.send_To_FIXGUN(sql_List_)
                        #     else:
                        #         print("send product name : ", sql_List_)
                        #         RCM_Server.send_To_FIXGUN(sql_List_)
                        #rd.old_sql_List = sql_List

                if check_Count == self.check_Count:
                    if len(rd.data_Tcp_Clinet_List) != 0:
                        print("Connect Client : ", rd.data_Tcp_Clinet_List)
                        rf.FLAG_ROBOT_START = True
                    else:
                        print("Not Connected")
                    check_Count = 0
                check_Count += 1
                if (self.robot_Connected == 1):
                    # if (self.sg_Connected == 1):
                    if (rf.FLAG_AUTO_MODE == False):
    ...
    def recv_From_Nsg(self, str_data):
        if (rf.FLAG_RECV_FROM_NSG_STOP == False):
            ...
            if (process_Status == 'S'):
                if (self.s_Counter == 0):
                    ...
                    pass
                self.s_Counter += 1
            elif (rf.FLAG_MOTION_EXECUTED == False):
                self.s_Counter = 0
                rf.FLAG_CUT_RECEIVE = False
                print("Received data from SG")
                print(" >", splitedNsgData)
            else:
                rf.FLAG_CUT_RECEIVE = True
                print("rf.FLAG_MOTION_EXECUTED : ", rf.FLAG_MOTION_EXECUTED)
            # else:
            #     rf.FLAG_CUT_RECEIVE = True
                    ...
                    if process_Status == 'P':
                        ...
                        if (product_Static_Speed != self.product_Static_Speed):
                            ...
                            if (rf.FLAG_FIRST_MOTION == False):
                                print("\n===================================")
                                print("Repeat motion : ", product_Name)
                                print("===================================\n")
                    ...
                    if (len(buffer) == 0):
                        ...
                        if (buffer == '1'):
                            ...
                            if (self.new_Product_Count > 0):
                                rf.FLAG_MOTION_EXECUTED = False
                ### FIXGUN # ================================================================================================= #
                elif deviceName == 'FIXGUN':
                    buffer = recv_data.decode()
                    print("recv buffer : ", buffer)
                    fixGunMotion = buffer #모션 저장 코드
                    converted_Data = rd.convert_Fixgun_Motion_From_Panel_PC(fixGunMotion)
                    print("recv data : ", converted_Data)
                    #self.rcm_sql.save_Pose_Data_To_DB(self.gcm_sql.send_Product_Name(), converted_Data)#DB 모션 저장 코드
                    self.rcm_sql.save_Pose_Data_To_DB('Change', converted_Data)#DB 모션 저장 코드
                ### NONAME # ================================================================================================== #
                else:
                    buffer = recv_data.decode()
    ...
    def run_Robot_By_Scheduler_Combined_Data(self, speed):
        ...
        if (self.product_Name in rd.sql_Datas):
            try:
                rd.way_To_Robot_Move = eval(self.rcm_sql.load_Pose_Data_With_Appearance_Rate_From_DB(self.product_Name))[2]
            except:
                rd.way_To_Robot_Move = 'L'
                print("no order to robot move")
            ...
            try:
                rd.way_To_Robot_Move = eval(self.rcm_sql.load_Pose_Data_With_Appearance_Rate_From_DB("DEFAULT"))[2]
            except:
                rd.way_To_Robot_Move = 'L'
                print("no order to robot move")
    ...
    def data_Load_Joint_Angle_List(self, loaded_Motion_Data, received_Speed):
        for joint_Angle in loaded_Motion_Data:
            rd.data_Draw_Ur_Pose_List.append(joint_Angle[:6])
            ### mm/s # =============================================================================================== #
            speed = float(joint_Angle[7] * (int(received_Speed) * 0.02))
            # speed = int(speed_ / 6) * 10
            # ======================================================================================================== #

            ### % # ================================================================================================== #
            # speed = int(joint_Angle[6] * (int(received_Speed) * 0.02))
            # if (speed > 99):
            #     speed = 99
            # elif (speed < 10):
            #     speed = "00"
            # ======================================================================================================== #
            rd.data_Draw_Robot_Speed_List.append(speed)
            try:
                rd.data_Draw_Ur_Gripper_Motion_List.append(joint_Angle[8])
            except:
                length = len(loaded_Motion_Data)
                if self.paint_Count < length - 1:
                    rd.data_Draw_Ur_Gripper_Motion_List.append('1')
                elif self.paint_Count == length - 1:
                    rd.data_Draw_Ur_Gripper_Motion_List.append('0')
                    self.paint_Count = 0
                self.paint_Count += 1
        rd.ghost_Robot_Angle_List = rd.data_Calculate_Angle_To_Kawasaki_Protocol(rd.data_Draw_Ur_Pose_List)
        speed = rd.data_Draw_Robot_Speed_List
        gripper = rd.data_Draw_Ur_Gripper_Motion_List
        rd.motion_Row_Number = len(rd.ghost_Robot_Angle_List)
        try:
            rd.data_Set_Combine_Angle_Data(rd.motion_Row_Number, rd.way_To_Robot_Move, rd.ghost_Robot_Angle_List,
                                           gripper, speed)
            print("FLAG_SIMULATION : ", rf.FLAG_SIMULATION)
    ...
    def Exception(self):
        ...
        loaded_Motion_Data = eval(self.rcm_sql.load_Pose_Data_With_Appearance_Rate_From_DB("DEFAULT"))[1]
        self.data_Load_Joint_Angle_List(loaded_Motion_Data, '50')

    def data_Reset(self):
        rd.combined_Joint_Angle_Data = []
        rd.ghost_Robot_Angle_List = []
        rd.data_Draw_Ur_Pose_List = []
        rd.data_Draw_Ur_Gripper_Motion_List = []
        rd.data_Draw_Robot_Speed_List = []
    ...
    def del_Client(self, deviceName):
        try:
            ...
            del rd.data_Tcp_Clinet_List[deviceName]
            self.client_Count -= 1
            self.lock.release()
        except Exception as e:
