...
class MysqlController:
    ...
    def loading_Insert(self, partID):
        sql = """INSERT INTO product_log (laneNumber,pcode,startTime) VALUES (%s,%s,%s)"""
        laneNumber = "1"
        now = datetime.datetime.now()
        nowDatetime = now.strftime('%Y-%m-%d %H:%M:%S')
        args = (laneNumber, partID, nowDatetime)
        self.curs.execute(sql, args)
        self.conn.commit()
        print("insert complete")

    def loading_Update(self, partID):
        sql = """SELECT idx FROM maviz.product_log WHERE pcode = %s ORDER BY idx DESC LIMIT 1"""
        args = (partID)
        self.curs.execute(sql, args)
