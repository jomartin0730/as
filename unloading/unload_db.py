...
class Ui_ImageDialog(QWidget):
    ...
    def createInfoGroup(self):
        ...
        self.ip = QLineEdit('172.17.0.220')
...
# 기능관련 #############################
    ...
    def Loading_db(self,partID):
        if self.old_partID is not None :
            if self.partID != self.old_partID : #새로운 제품 들어올 경우
                self.d.loading_Insert(self.partID) #새로운 제품 삽입
                self.d.loading_Update(self.old_partID) #이전 제품 마감
...
# 슬랏  #####################################
    ...
    @pyqtSlot(str,str)
    def update_imfomsg(self,partID, amount):
        self.partID = partID
        now = datetime.datetime.now()
        nowDatetime = now.strftime('%Y-%m-%d %H:%M:%S')
        _str = "제품 명 : " + partID + ", 수량 : " + amount + ", " + nowDatetime
        self.Loading_db(partID)
