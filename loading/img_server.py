...
class xavier_init() :
    def __init__(self):
        ...
        self.camera_A_flag = False
        self.camera_B_flag = False
        self.A_detect_flag = False
        self.camera_state = False

        self.camera_A_data = None
        self.camera_B_data = None
        self.detect_data = None
        self.img = None

        self.d = MysqlController.MysqlController('192.9.226.32', 'mgt', 'aA!12345', 'maviz')
    ...
    def detect(self, imgSrc):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if self.old_partID is None:
                self.old_partID = self.partID

            if len(self.partID) < 4:
                self.partID = self.partID
            elif len(self.partID) < 12:
                self.partID = self.partID [:3] + '-' + self.partID [3:5] + "-" + self.partID [5:]
            else:
                self.partID = self.partID
            ...
            if self.percent >= 0.85:
                if self.old_partID is not None:
                    if self.partID != self.old_partID:  # 새로운 제품 들어올 경우
                        self.d.loading_Insert(self.partID, self.percent)  # 새로운 제품 삽입
                        if self.partID == "END":
                            self.d.loading_Update(self.partID, self.percent)  # 이전 제품 마감
                        self.old_partID = self.partID
        ...
        return returnData, im0, pname, detect_check, self.percent
        ...
        while device_bool is True :
            ...
            if grabResult.GrabSucceeded():
                ...
                if color_a is not None and color_b is not None:
                    dst, move_check, goods_check = self.active_check(first_frame, color_a)
                    first_frame = color_a
                    ...
                    a_raw = self.color_histo(a_raw)
                    a_histo = self.color_histo(color_a)
                    b_histo = self.color_histo(color_b)
                    ...
                    # print("size : " ,size_c)
                    self.camera_A_data = ctype + color_state + (str(len(color_stringData))).encode().ljust(16) + color_stringData
                    self.camera_A_flag = True
                    #clnt_sock.sendall(ctype + color_state + (str(len(color_stringData))).encode().ljust(16) + color_stringData)
                    ...
                    self.camera_B_data = ctype + colorb_state + (str(len(colorb_stringData))).encode().ljust(16) + colorb_stringData
                    self.camera_B_flag = True
                    #clnt_sock.sendall(ctype + colorb_state + (str(len(colorb_stringData))).encode().ljust(16) + colorb_stringData)
    ...
    def handle_client(self,clnt_sock, addr) :
        while clnt_sock is not None:
            if self.camera_state == True :
                if self.camera_A_flag == True and self.camera_B_flag == True:
                    if self.camera_A_data is not None and self.camera_B_data is not None:
                        clnt_sock.sendall(self.camera_A_data)
                        self.camera_A_flag = False
                        clnt_sock.sendall(self.camera_B_data)
                        self.camera_B_flag = False
