...
class RcmServerSignal(QObject):
    ...
    red_alarm_signal = pyqtSignal(str)
    yellow_alarm_signal = pyqtSignal(str)
    green_alarm_signal = pyqtSignal(str)
...
class RCM_Server(threading.Thread):
    ...
    def recv_From_Nsg(self, str_data, start_flag):
        if(rf.FLAG_RECV_FROM_NSG_STOP == False):
            ...
            if (rf.FLAG_RECV_FROM_NSG_PRODUCT == True):
                product_Name_ = splitedNsgData[0]
                if (len(product_Name_) == 0):
                    product_Name_ = "None"
            else:    
                product_Name_ = rd.rcm_product_Name
            print("rd.rcm_product_Name : ", rd.rcm_product_Name)
            print("product name_ :", product_Name_)
                ...
                if deviceName == 'ROBOT':
                    ...
                    if (len(buffer) == 0):
                        ...
                        if (buffer == '1'):
                            ...
                            if (buffer == "R"): 
                                self.rcm_server_signal.red_alarm_signal.emit("R")
                                pass
                            elif (buffer == "Y"):
                                self.rcm_server_signal.yellow_alarm_signal.emit("Y")
                                pass
                            elif (buffer == "G"):
                                self.rcm_server_signal.green_alarm_signal.emit("G")
                                pass
                            else:
                                if (rf.FLAG_RECV_FROM_NSG_STOP == False): # 로봇 자동 동작
                                    print("a",f"{self.endTime - self.rcmStartTime:.5f} sec")
